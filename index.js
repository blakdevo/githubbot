...
client.once('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder().setName('clear').setDescription('Clear all messages + remove rent system'),
    new SlashCommandBuilder().setName('book').setDescription('Make a reservation'),
    new SlashCommandBuilder().setName('setupreservation').setDescription('Setup permanent reservation button (Admin only)'),
    new SlashCommandBuilder().setName('setupsignup').setDescription('Setup verification message (Admin only)'),
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'event') {
      ...
      if (!startTime || startTime < Date.now() + 60000) {
        return interaction.editReply({ content: 'Geçersiz veya geçmiş zaman! Örnek: `3 saat sonra`, `22:30`, `yarın 21:00`' });
      }
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId.startsWith('event_settings_')) {
      ...
      const manualStart = new ButtonBuilder().setCustomId(`manual_start_${eventId}`).setLabel('Manuel Başlat').setStyle(ButtonStyle.Success);
      const cancelEvent = new ButtonBuilder().setCustomId(`cancel_event_${eventId}`).setLabel('İptal Et').setStyle(ButtonStyle.Danger);
    ...
    if (interaction.customId.startsWith('manual_start_')) {
      ...
      await interaction.update({ content: '✅ Event manuel olarak başlatıldı!', components: [] });
    }
...
function parseTimeInput(input) {
  ...
  if (input.includes('tomorrow') || input.includes('yarın')) {
    ...
    if (m && m.length > 2) {
       date.setHours(parseInt(m[1]), parseInt(m[2]), 0, 0);
    } else if (m) {
       date.setHours(parseInt(m[1]), 0, 0, 0);
    } else {
...
async function autoStartRentSystem(eventId) {
  ...
  await channel.send(`**EVENT BAŞLADI!** "${event.name}"\nKatılımcılar: ${event.participants.map(id => `<@${id}>`).join(', ') || 'Kimse katılmadı'}\nRent sistemi aktif ediliyor...`);

  if (RENT_SYSTEMS.has(channel.id)) {
    const old = RENT_SYSTEMS.get(channel.id);
